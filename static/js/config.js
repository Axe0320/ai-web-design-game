export const DEFAULTS = {
    bgColor: '#ffffff',
    textColor: '#1e293b',
    titleColor: '#0f172a',
    cardColor: '#f1f5f9',
    cardTitleColor: '#0f172a',
    cardTextColor: '#475569',
    titleSize: '48px',
    fontSize: '20px',
    lineHeight: '1.7',
    imgWidth: '100%',     // IMG_WIDTH_MAP[5]
    imgHeight: '300px',    // IMG_HEIGHT_MAP[3]
    cardTitleSize: '24px',
    cardFontSize: '20px',
    layout: 'center',
    padding: '60px'
};

export const SOFT_COLORS = [
    { name: '白', value: '#ffffff' }, { name: '紺', value: '#1e293b' }, { name: '灰', value: '#94a3b8' },
    { name: '緑', value: '#86efac' }, { name: '青', value: '#93c5fd' }, { name: '紫', value: '#c4b5fd' },
    { name: '赤', value: '#fda4af' }, { name: '橙', value: '#fdba74' }, { name: '黄', value: '#fef08a' },
    { name: '砂', value: '#f5f5f4' }
];

export const SIZE_MAP = { "1": "14px", "2": "16px", "3": "20px", "4": "24px", "5": "32px" };
export const TITLE_SIZE_MAP = { "1": "24px", "2": "32px", "3": "48px", "4": "64px", "5": "80px" };
export const CARD_TITLE_SIZE_MAP = { "1": "16px", "2": "20px", "3": "24px", "4": "28px", "5": "36px" };
export const LINE_MAP = { "1": "1.2", "2": "1.4", "3": "1.7", "4": "2.1", "5": "2.6" };
export const PADDING_MAP = { "1": "15px", "2": "30px", "3": "60px", "4": "100px", "5": "150px" };
export const IMG_WIDTH_MAP = { "1": "30%", "2": "50%", "3": "70%", "4": "90%", "5": "100%" };
export const IMG_HEIGHT_MAP = { "1": "100px", "2": "200px", "3": "300px", "4": "450px", "5": "600px" };
export const LABEL_MAP = { "1": "極小", "2": "小", "3": "標準", "4": "大", "5": "最大" };

// SHA-256 hash of the settings password
export const SETTINGS_PASSWORD_HASH = '90b5b7266e807593aa3debe1badb70790e96b08e5835dbc03b81f4605b653b08';
