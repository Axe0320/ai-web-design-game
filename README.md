...
### 3.4 フロントエンド（Vanilla JS / ES Modules）
...
`vercel.json` が設定済みのため、GitHubリポジトリと連携するだけでデプロイできます。

Vercel の環境変数設定画面（Settings → Environment Variables）で以下を登録してください。
...
1. `prompt_{カテゴリ名}.txt` を作成
2. `static/js/app.js` の `categories` 配列に追加
3. `static/pages/evaluation.html` に対応するカードを追加
...
* 本番運用前に `.env` の `GEMINI_API_KEY`（開発用）を本番用キーに差し替えてください
